...
> 当前版本、依赖、构建、部署和发布统一由 [`versions.toml`](versions.toml) 与
> [`tools/lmlp`](tools/lmlp) 管理。维护流程见
...
<table>
  ...
  <tr>
    ...
    <td align="left"><code>Shift + 单击</code>材料行打开完整配方详情页，复用 JEI 原生配方布局。</td>
  </tr>
  <tr>
...
详情页用于查看一个材料的所有可用配方来源。它会尽量复用 JEI 原生配方布局、物品交互、tooltip 和配方转移按钮，因此工作台、切石机、高炉等不同类型的配方都能保持接近 JEI 的展示方式。
...
- 展开和收起带高度动画。
- 使用材料列表热键返回原加工界面。
- 从加工界面进入时，可以使用 JEI 转移逻辑把材料摆入当前容器。
...
<table>
  ...
  <tr>
    ...
    <td align="center">单表树状层级结构，展示每个主材料和它向下拆出的递归子材料。</td>
  </tr>
  <tr>
    <td align="center"><code>Sub-material Totals</code></td>
    <td align="center">汇总所有最小子材料的总数、缺少和可用数量。</td>
  </tr>
</table>
...
XLSX 会保留当前配置选择的数量显示样式，并包含表头样式、首行冻结、筛选、自动列宽。汇总表还提供数值类型的 `Raw Total` 列，便于按未换算前的材料总数筛选和排序。
...
3. 确认同一实例中已经安装 Fabric API、Litematica、MaLiLib 和 JEI。
4. 启动游戏，打开 Litematica 材料列表。

## 兼容环境

LMLP 按 Minecraft 版本分别发布。请下载与你的 Minecraft 版本完全对应的文件。

<div align="center">

| Minecraft | LMLP 版本 | Java |
| --- | --- | --- |
| `26.1.2` | `1.9.0+mc26.1.2` | `25` |
| `1.21.11` | `1.9.0+mc1.21.11` | `21` |
| `1.21.10` | `1.9.0+mc1.21.10` | `21` |
| `1.21.1` | `1.9.0+mc1.21.1` | `21` |
| `1.20.6` | `1.9.0+mc1.20.6` | `21` |
| `1.20.4` | `1.9.0+mc1.20.4` | `17` |
| `1.20.1` | `1.9.0+mc1.20.1` | `17` |

</div>

JEI 是 v1.8.0 起的必要前置。Minecraft 1.21.4 和 1.21.5 因没有对应的受支持 JEI 目标，最后可用版本为 LMLP v1.7.0。

<details>
<summary>推荐前置版本</summary>

下表为作者实际测试使用的前置版本。通常情况下，同一 Minecraft 版本下的更高小版本也可以尝试使用。

| Minecraft | Fabric API | Litematica | MaLiLib | JEI |
| --- | --- | --- | --- | --- |
| `26.1.2` | `0.154.2+26.1.2` | `0.27.10` | `0.28.9` | `29.16.0.47` |
| `1.21.11` | `0.141.4+1.21.11` | `0.26.12` | `0.27.16` | `27.17.0.50` |
| `1.21.10` | `0.138.4+1.21.10` | `0.24.9` | `0.26.8` | `26.3.0.31` |
| `1.21.1` | `0.116.12+1.21.1` | `0.19.60` | `0.21.10` | `19.38.0.366` |
| `1.20.6` | `0.100.8+1.20.6` | `0.18.3` | `0.19.2` | `18.0.0.66` |
| `1.20.4` | `0.97.3+1.20.4` | `0.17.4` | `0.18.4-alpha.1` | `17.3.1.5` |
| `1.20.1` | `0.92.9+1.20.1` | `0.15.4` | `0.16.3` | `15.20.0.134` |

</details>

**如果出现启动失败、材料列表异常或 JEI 配方显示异常，请先确认依赖版本；若问题仍然存在，请运行 `tools/lmlp debug-bundle` 或 Windows 的 `tools/collect-debug.ps1` 收集调试信息。**

## 分支说明

- `main` 保存项目介绍、版本清单、统一管理工具和 Release 流程。
- `mc1.xx.x / mc26.x.x` 分别保存对应 Minecraft 版本源码，修改时互不影响。
- 正式版本发布后不覆盖 Tag 或附件；修复使用新的补丁版本号。

## 未来展望

- 适配更多版本的 Minecraft，最低版本或许下探 1.18.2，最高跟随 Minecraft 和 Litematica 更新。
- 跟随 JEI、Litematica 和 MaLiLib 的新版本继续适配。
- 对游戏内 HUD 增强显示。
- 更新频率取决于作者学业强度、 Codex 剩余额度以及 Litematica 更新强度。

## 许可证

本项目使用 LGPL-3.0-or-later 发布。Litematica、MaLiLib、JEI、Fabric API 等依赖模组遵循其各自许可证，本项目不随 jar 打包这些依赖。
