...
- 深层循环检测只沿单一材料转换继续追踪；多输入加工配方常把可复用工具或容器暴露为普通 INPUT，若沿完整合成链搜索，会产生“木板 → 去皮原木 → 斧头 → 木板”一类假循环。直接自引用仍必须立即终止。
- 白色床、煤炭块等不应因为存在可逆或替代配方而被错误继续拆分。
- “任意原木”只展开木材选择组，不要把选择组递归解释成煤炭块等目标的通用原料。
- 原木、木头、菌柄、菌核和竹块属于木材分解链的终端资源，不继续解析 JEI 中的剥皮/切割配方及其工具槽。
...
- JEI 查询必须按分类和单条配方隔离异常；一个第三方插件的坏 layout 只能跳过自身，不能让已找到的原版配方整批丢失并缓存为空。
