...
import io.github.huanmeng06.lmlp.mixin.ClientRecipeBookAccessor;
    ...
    private record TransferContext(IRecipeTransferHandler handler, class_1703 menu, JeiRecipeResolver.JeiNativeRecipe nativeRecipe) {
        ...
        private IRecipeTransferError transfer(int craftsMissing, boolean maxTransfer) {
            ...
            if ((error == null || error.getType().allowsTransfer)
                ...
                this.recipeDisplayId().ifPresent(displayId -> {
        ...
        private Optional<class_10298> recipeDisplayId() {
            if (!(this.nativeRecipe.recipe() instanceof class_8786<?> holder)
                    || !(holder.comp_1933() instanceof class_1860<?> recipe)
                    || class_310.method_1551().field_1724 == null) {
                return Optional.empty();
            }

            List<class_10295> displays = recipe.method_64664();
            Map<class_10298, class_10297> entries =
                    ((ClientRecipeBookAccessor) class_310.method_1551().field_1724.method_3130())
                            .lmlp$getDisplayEntries();
            return entries.entrySet().stream()
                    .filter(entry -> displays.contains(entry.getValue().comp_3263()))
