...
public class GuiPreferredSchematicSave extends GuiSchematicSave {
    ...
    private void saveNewCopy() {
        ...
        File directory = this.getListWidget().getCurrentDirectory();
        String name = safeFileName(this.getTextFieldText());
        if (!Files.isDirectory(directory.toPath())) {
            this.addMessage(MessageType.ERROR, "litematica.error.schematic_save.invalid_directory", directory.getAbsolutePath());
            return;
        }
        if (name.isEmpty()) {
        ...
        Path destination = directory.toPath().resolve(fileName);
    ...
    private static String safeFileName(String input) {
        ...
        return input.trim().replaceAll("[\\\\/:*?\"<>|\\p{Cntrl}]", "_");
    }
}
