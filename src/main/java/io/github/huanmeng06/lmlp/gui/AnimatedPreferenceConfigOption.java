...
final class AnimatedPreferenceConfigOption extends WidgetConfigOption {
    ...
    private void renderSelectedMaterialIcon(GuiContext context) {
        ...
        Identifier itemId = Identifier.parse(definition.selectedItemId());
        Item item = BuiltInRegistries.ITEM.getOptional(itemId).orElse(null);
        if (item == null) {
            return;
        }

        int resetWidth = this.textRenderer.width(StringUtils.translate("malilib.gui.button.reset.caps")) + 10;
