...
public final class PlacementOriginMarker {
    ...
    private static final int LABEL_LIGHT_U = LABEL_LIGHT & 0xFFFF;
    private static final int LABEL_LIGHT_V = LABEL_LIGHT >> 16 & 0xFFFF;
    ...
    public static void render(WorldRenderContext context) {
        ...
        drawBeam(context.matrixStack(), context.consumers(), target.pos, cameraPos);
        if (target.sameDimension && client.field_1724 != null) {
            drawTargetInfo(context.matrixStack(), context.consumers(), context.camera(), client, current, target.pos);
        }
    }
    ...
    private static void drawBeam(class_4587 matrices, class_4597 consumers, class_2338 pos, class_243 cameraPos) {
        ...
        drawBoxQuads(consumers.getBuffer(class_1921.method_49042()), matrix, minX, minY, minZ, maxX, maxY, maxZ, BEAM_FILL);
        class_761.method_22980(matrices, consumers.getBuffer(class_1921.method_23594()), minX, minY, minZ, maxX, maxY, maxZ,
                BEAM_OUTLINE.r, BEAM_OUTLINE.g, BEAM_OUTLINE.b, BEAM_OUTLINE.a);
        matrices.method_22909();
    }

    private static void drawBoxQuads(class_4588 buffer, Matrix4f matrix, float minX, float minY, float minZ, float maxX, float maxY, float maxZ, Color4f color) {
    ...
    private static void quad(class_4588 buffer, Matrix4f matrix,
    ...
    private static void drawTargetInfo(class_4587 matrices, class_4597 consumers, net.minecraft.class_4184 camera, class_310 client, Marker current, class_2338 pos) {
        ...
        Matrix4f matrix = matrices.method_23760().method_23761();
        drawTargetTexture(matrix, consumers, fade);
        if (pointedAt) {
            matrices.method_22903();
            float textScale = originMarkerTextScale();
            matrices.method_22905(textScale, textScale, 1.0F);
            drawLabel(matrices.method_23760().method_23761(), consumers, textRenderer, title, coordinate, distanceText, fade);
            matrices.method_22909();
        }
        matrices.method_22909();
    }
    ...
    private static void drawTargetTexture(Matrix4f matrix, class_4597 consumers, float alpha) {
        class_4588 buffer = consumers.getBuffer(class_1921.method_37345(TARGET_TEXTURE));
    ...
    private static void drawLabel(Matrix4f matrix, class_4597 consumers, class_327 textRenderer, String title, String coordinate, String distanceText, float alpha) {
        ...
        textRenderer.method_27522(title, -titleWidth / 2.0F, LABEL_ELEVATE_BY, color, false, matrix, consumers, class_327.class_6415.field_33994, 0, LABEL_LIGHT, false);
        textRenderer.method_27522(coordinate, -coordinateWidth / 2.0F, LABEL_ELEVATE_BY + LABEL_LINE_HEIGHT, color, false, matrix, consumers, class_327.class_6415.field_33994, 0, LABEL_LIGHT, false);
        textRenderer.method_27522(distanceText, -distanceWidth / 2.0F, LABEL_ELEVATE_BY + LABEL_LINE_HEIGHT * 2, color, false, matrix, consumers, class_327.class_6415.field_33994, 0, LABEL_LIGHT, false);
    }

    private static void rectangle(class_4588 buffer, Matrix4f matrix, int x1, int y1, int x2, int y2, float r, float g, float b, float a) {
    ...
    private static void colorVertex(class_4588 buffer, Matrix4f matrix, float x, float y, float r, float g, float b, float a) {
        buffer.method_22918(matrix, x, y, 0.0F).method_22915(r, g, b, a).method_1344();
    }

    private static void colorVertex(class_4588 buffer, Matrix4f matrix, float x, float y, float z, Color4f color) {
