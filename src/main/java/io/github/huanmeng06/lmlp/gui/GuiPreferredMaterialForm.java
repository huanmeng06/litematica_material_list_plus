...
public final class GuiPreferredMaterialForm extends GuiConfigsBase {
    ...
    public boolean onMouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (overVisibleEntries
                ...
                && this.clickDetailRowButton(mouseX, mouseY, mouseButton)) {
            return true;
        }
    ...
    @Override
    public void drawContents(class_332 context, int mouseX, int mouseY, float partialTicks) {
        this.updateBulkActionButtons();
    ...
    private void renderDetailRow(
        ...
        RenderUtils.drawRect(rowLeft, y, rowRight - rowLeft, DETAIL_ROW_HEIGHT, background);

        int sourceIconX = rowLeft + 4;
        int iconY = y + 3;
        if (row.row.sourceBlock() != null) {
            RenderUtils.drawRect(sourceIconX, iconY, 16, 16, DETAIL_ICON_BACKGROUND);
        ...
        row.button.render(mouseX, mouseY, row.button.isMouseOver(), context);
    ...
    private void renderEmptyDetailRow(class_332 context, String message, int y, int mouseX, int mouseY) {
        int rowLeft = DETAIL_MARGIN;
        int rowRight = this.field_22789 - DETAIL_MARGIN - 12;
    ...
    private boolean clickDetailRowButton(int mouseX, int mouseY, int mouseButton) {
        for (RowState row : this.renderedRows) {
            if (this.detailsExpanded.getOrDefault(row.row.category(), false)
                    && row.button.onMouseClicked(mouseX, mouseY, mouseButton)) {
                return true;
            }
        }
        return false;
    ...
    private void confirm() {
        ...
        Path source = this.placement.getSchematicFile() == null
                ? null
                : this.placement.getSchematicFile().toPath();
    ...
    private final class RowState {
        ...
        private void selectTarget(String targetId) {
            class_2248 selected = class_7923.field_41175
                    .method_17966(new class_2960(targetId))
                    .orElse(null);
            if (selected == null) {
                return;
            }
